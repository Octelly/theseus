...
pub async fn auth_authenticate_begin_flow() -> Result<url::Url> {
    Ok(hydra::authenticate_begin_flow().await?)
}
...
#[tauri::command]
pub async fn auth_authenticate_await_completion() -> Result<HydraCredentials> {
    Ok(hydra::authenticate_await_complete_flow().await?)
}

#[tauri::command]
pub async fn auth_cancel_flow() -> Result<()> {
    Ok(hydra::cancel_flow().await?)
}

/// Refresh some credentials using Hydra, if needed
// invoke('plugin:auth|auth_refresh',user)
#[tauri::command]
pub async fn auth_refresh(user: uuid::Uuid) -> Result<HydraCredentials> {
    Ok(hydra::refresh(user).await?)
}

#[tauri::command]
pub async fn auth_remove_user(user: uuid::Uuid) -> Result<()> {
    Ok(hydra::remove_user(user).await?)
}
...
pub async fn auth_has_user(user: uuid::Uuid) -> Result<bool> {
    Ok(hydra::has_user(user).await?)
}

/// Get a copy of the list of all user credentials
// invoke('plugin:auth|auth_users',user)
#[tauri::command]
pub async fn auth_users() -> Result<Vec<HydraCredentials>> {
    Ok(hydra::users().await?)
}
...
// invoke('plugin:auth|auth_users',user)
#[tauri::command]
pub async fn auth_get_user(user: uuid::Uuid) -> Result<HydraCredentials> {
    Ok(hydra::get_user(user).await?)
}
